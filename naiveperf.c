...
static const event_alias profile_events[] = {
    ...
    {
        "retired loads that missed L1 d-cache", {
          "L1D_CACHE_MISS_LD_NONSPEC",
    }},
    {
        "dirty cache lines written back from L1D cache to shared L2 cache", {
          "L1D_CACHE_WRITEBACK",
    }},
//    {
//        "retired stores that missed in the L1 D cache", {
...
void convertToMorton(double *mat, double *mort, int size) {
  for (int r = 0; r < size; r++) {
    for (int c = 0; c < size; c++) {
      int res = layout(r, c);
      mort[res] = mat[r * size + c];
    }
  }
}

void convertFromMorton(double *mat, double *mort, int size) {
  for (int r = 0; r < size; r++) {
    for (int c = 0; c < size; c++) {
      int res = layout(r, c);
      mat[res] = mort[r * size + c];
    }
  }
}

bool power_of_two (int x) { return (x & (x - 1)) == 0; }
